...
exports.default = () => {
    passport_1.default.use(new passport_local_1.default.Strategy({
        ...
        try {
            const user = yield models_1.User.findOne({ where: { email }, include: [models_1.Token] });
    ...
    passport_1.default.use(new passport_jwt_1.default.Strategy({
        ...
        try {
            const user = yield models_1.User.findOne({ where: { id: jwt.id }, include: [models_1.Token] });
            if (!user) {
                done({ user: 'Not found' });
            }
            else {
